...
function getCk3Cultures() {
    ...
    for CULTURES_DIR in "${@}"; do
        for CULTURE_ID in $(grep -P '^\s*name_list\s*=' "${CULTURES_DIR}/"*.txt | \
            ...
            if $(echo 'color dlc_tradition ethnicities parents placeholder traditions' | grep -q "${CULTURE_ID}"); then
                continue
            fi

            [[ ${CULTURE_ID} =~ _gfx$ ]] && continue
...
function getHoi4Countries() {
    ...
    for TAG in $(cat "${TAGS_DIR}/"*.txt | awk -F"=" '{print $1}' | sed 's/\s*//g' | sort | uniq | grep -v "^#"); do
        COUNTRY_NAME=$(getHoi4CountryName "${TAG}" "${LOCALISATIONS_DIR}")
...
echo "Crusader Kings 3 CMH:"    && getCk3Cultures       "CK3CMH"    "${CK3CE_CULTURES_DIR}" "${CK3IBL_CULTURES_DIR}" "${CK3RICE_CULTURES_DIR}" "${CK3_CULTURES_DIR}"
...
echo "Crusader Kings 3 TBA:"    && getCk3Cultures       "CK3TBA"    "${CK3TBA_CULTURES_DIR}"
...
echo "Hearts of Iron 4 RT56:"   && getHoi4Countries     "HOI4RT56"  "${HOI4RT56_TAGS_DIR}" "${HOI4RT56_LOCALISATIONS_DIR}"
