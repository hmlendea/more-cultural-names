...
function normalise-name() {
    ...
    local P_PREFECTURE="[Pp]r[aäeé]fe[ckt]t[uúū]r[ae]*"
    ...
    local NORMALISED_NAME=$(echo "${TRANSLITERATED_NAME}" | \
        ...
        sed \
            ...
            -e 's/[ ’-]\('"${P_ANCIENT}"'\|'"${P_CASTLE}"'\|'"${P_CATHEDRAL}"'\|'"${P_GMINA}"'\|'"${P_PREFECTURE}"'\|'"${P_REGION}"'\)$//g' \
            -e 's/ [KkCc]om*un*[ea]*$//g' \
            -e 's/ \(Ken\|Koān\)$//g' \
            \
            -e 's/^Hl\. /Heilige /g' \
