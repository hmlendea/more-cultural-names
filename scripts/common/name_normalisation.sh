...
function normalise-name() {
    ...
    local NORMALISED_NAME=$(echo "${TRANSLITERATED_NAME}" | \
        ...
        sed \
            ...
            -e 's/\(n\|ṉ\) [PpP‍p‍]e\(n\|[ṉ]*\)i\(n\|[ṉ]*\)[cs]ulā$//g' \
            -e 's/n dili$//g' \
            -e 's/n kieli$//g' \
            -e 's/n kreivikunta$//g' \
            -e 's/nag ævzag$//g' \
            ...
            -e 's/^[Ll][ei]ngua //g' \
            -e 's/^D Regioon //g' \
            -e 's/^\([Ee]*[SsŠš]ta[dt][eou]*[l]*\|Jangwa\|Kongedømmet\|[Pp]ar[r]*o[i]*\(cc\|q\|ss\)\(e\|[hu]ia\)\|[PpP‍p‍]en[ií]n[cs]ul[aā]\|[Pp][’]*r[ao][bpvw][ëií][nñ][t]*[csz]*[eiíjoy]*[aez]*\|Sultanat\) \([dl]*[aeiou]*[ '"'"']\|of\)*//g' \
