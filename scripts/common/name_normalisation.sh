...
function normalise-name() {
    ...
    local P_STATE="Bang\|[EeÉé]*[SsŜŝŜŝŠšŞş]*[h]*[tṭ][’]*[aeē][dtṭu][’]*[aeiıosu]*[l]*\|valstija*"
    ...
    local NORMALISED_NAME=$(echo "${TRANSLITERATED_NAME}" | \
        ...
        sed \
            ...
            -e 's/^[Ll][ei]ngua //g' \
            -e 's/^D '"${P_REGION}"' //g' \
            -e 's/^\('"${P_ANCIENT}"'\|'"${P_CASTLE}"'\|'"${P_CATHEDRAL}"'\|'"${P_COUNTRY}"'\|'"${P_DISTRICT}"'\|'"${P_GMINA}"'\|'"${P_PREFECTURE}"'\|'"${P_REGION}"'\|'"${P_STATE}"'\)\s\s*\(\([dl]*[aeiou]*[l]*\|of\|ng\|[Tt]a\|[Tt]o[uy]\|w\)[ '"'"']\)*//g' \
            ...
            -e 's/[ ’-]\([CcKk]o[ou]*nt\(ae\|luğu\|y\)\|[Dd]epart[a]*ment\(as\)*\|Durg[a]*\|[EeIi]*[Ss][h]*t[’]*a[dt][’]*[eiou]\|[Ff]ort[r]*e[t]*s[s]*[y]*[a]*\|[Ff]ylk[ei]\|[GgQq]r[aā]f[l]*[iı]\(ı\|ste\)\|\|[Jj][eēi]l[hl]*[aāeo][a]*\|[Jj][oō][u]*\|[Kk]il[l]*[ao]\|[Mm]a[nṇ][dḍ]al\(a[mṁ]\)*\|Muni\(c\|ts\)ip[’]*alit[’]*\(et’i\|y\)\|[Nn]ahan[gk]\|P[aā][i]*ri\(ṣ\|sh\)\|[Mm]in[tţ]a[kq]at*\|[Oo]blas[tť]\|[Pp][ao][luŭ][uv]ostr[ao][uŭv]\|[Pp][eé]n[iíì][n]*[csz][ou][lł][aāe]\|[Pp]ii\(r\|skop\)kond\|[Pp]lanin[ai]\|[Pp]r[a]*d[eē][sś][h]*[a]*\|[Pp]r[aā]*nt[y]*[a]*\([ṁy][a]*\)*\|[Pp][’]*r[aāou][bpvw][ëií][nñ][t]*[csz]*[eėiíjoy]*[aeėsz]\|[Pp]rja[a]*st[t]*a[a]*k\|[Pp]u[u]*[l]*s[s]*a[al][ar]\|[Rr]eialme\|[Rr][e]*[iī]c[eh]\|[Rr]esp[uúy][’]*bli[’]*[ck][ai]\(sy\)*\|[Ss]h[eě]ng\|[Vv]il[aā][jy]\(eti\|s\)\|[ZzŽž]upa\(nija\)*\)$//g' \
            -e 's/[ ’-]\('"${P_ANCIENT}"'\|'"${P_CASTLE}"'\|'"${P_CATHEDRAL}"'\|'"${P_COUNTRY}"'\|'"${P_DISTRICT}"'\|'"${P_GMINA}"'\|'"${P_PREFECTURE}"'\|'"${P_REGION}"'\|'"${P_STATE}"'\)$//g' \
            -e 's/ [KkCc]om*un*[ea]*$//g' \
            -e 's/ \(Ken\|Koān\)$//g' \
            \
            -e 's/^Hl\. /Heilige /g' \
